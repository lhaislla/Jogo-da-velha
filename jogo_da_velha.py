...
def imprime_tabuleiro():
    ...
    """
    # Limpa o terminal quando função é chamada:
...
# Verificação se o tabuleiro está completo
...
def mensagem_final_jogo(jogador):
    ...
    print('=' * 30)
# Analise do status do jogo 
def analisando_jogo(jogada):
    """
    AO FINAL DO JOGO GANHA QUEM COMPLETAR UMA RETA (LINHA OU COLUNA) COM 'X' OU 'O'
    OPÇÕES DE VITÓRIA NA MATRIZ:
        ...
        - 3, 5, 7 == [0, 2], [1, 1], [2, 0]
    """
    # Validação das vitórias possíveis de "X" ou "O":
...
def inicio_jogo():
    ...
    while True:
        ...
        # Validação se jogada é possível
        analisando_jogada(posicao_jogada, jogada)

        # verificação  se houve ganhador
        if analisando_jogo(jogada):
            break
        #Verificando se  valores da matriz foram preenchidos. Se for True, quer dizer empate
